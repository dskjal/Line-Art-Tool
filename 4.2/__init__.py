...
def create_lineart_grease_pencil():
    ...
    thick.use_normalized_thickness = True
...
class DSKJAL_OT_LINEART_TOOL_ADD_MODIFIER(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if self.modifier_type == 'GP_THICK':
            m.use_normalized_thickness = True
        filter_source = get_filter_source()
...
class DSKJAL_PT_LINEART_TOOL_UI(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        if my_props.lineart_ui_is_open:
            ...
            row.use_property_split = False
            row.prop(active_lineart, 'use_contour', text='')
            row.prop(active_lineart, 'silhouette_filtering', text='')
            ...
            row = box.row(align=True)
            row.use_property_split = False
            if getattr(active_lineart, 'use_loose', False):
                row.prop(active_lineart, 'use_loose', text='Loose', toggle=1)

            row = box.row(align=True)
            row.use_property_split = False
            row.prop(active_lineart, 'use_light_contour', text='Light Contour', toggle=1)
            row.prop(active_lineart, 'use_shadow', text='Cast Shadow', toggle=1)
            ...
            if getattr(active_lineart, 'use_face_mark', False):
                box.separator()
                row = box.row(align=True)
                row.use_property_split = False
                ...
                if is_edit_mode:
                    ...
                    ot.clear = True
            if getattr(active_lineart, 'use_overlap_edge_type_support', False):
                box.separator()
                row = box.row(align=True)
                row.use_property_split = False
        ...
        if my_props.thickness_is_open:
            col.prop(bpy.context.scene.lineart_tool_props.gp_object.data, 'stroke_thickness_space', text='Stroke Thickness')
        ...
        row.use_property_split = False
        row.alignment = 'LEFT'
        row.prop(my_props, 'composition_is_open', text='Composition', icon='TRIA_DOWN' if my_props.composition_is_open else 'TRIA_RIGHT', emboss=False)
        if my_props.composition_is_open:
            row = col.row(align=True)
            row.label(text='Depth Offset')
            row.prop(active_lineart, 'stroke_depth_offset', text='')
...
class DSKJAL_LINEART_TOOL_PROPS(bpy.types.PropertyGroup):
    ...
    composition_is_open : bpy.props.BoolProperty(name='composition_is_open', default=True)
...
def unregister():
    if getattr(bpy.types.Scene, "lineart_tool_props", False): del bpy.types.Scene.lineart_tool_props
    for cls in classes:
